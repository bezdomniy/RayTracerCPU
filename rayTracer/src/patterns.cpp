...
glm::vec3 StripedPattern::patternAt(glm::vec4 point) {
    if ((int)(std::floor(point.x) ) % 2 == 0) return this->colourA;
    return this->colourB;
}

GradientPattern::GradientPattern(glm::vec3 colourA, glm::vec3 colourB): Pattern()
{
    this->colourA = colourA;
    this->colourB = colourB;
}
...
glm::vec3 RingPattern::patternAt(glm::vec4 point) {
    if ((int)std::floor(std::sqrt(point.x * point.x + point.z * point.z)) % 2 == 0) return this->colourA;
    return this->colourB;
}

CheckedPattern::CheckedPattern(glm::vec3 colourA, glm::vec3 colourB): Pattern()
{
    this->colourA = colourA;
    this->colourB = colourB;
}
...
glm::vec3 CheckedPattern::patternAt(glm::vec4 point) {
    if (((int)(std::floor(point.x) + std::floor(point.y) + std::floor(point.z))) % 2 == 0) return this->colourA;
    return this->colourB;
}

BlendedPattern::BlendedPattern(std::shared_ptr<Pattern>& patternA, std::shared_ptr<Pattern>& patternB): Pattern()
{
...
glm::vec3 BlendedPattern::patternAt(glm::vec4 point) {
    ...
    return (this->patternA->patternAt(patternPointA) + this->patternB->patternAt(patternPointB)) * 0.5f;
}
