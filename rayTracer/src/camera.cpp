...
Ray Camera::rayForPixel(float px, float py) {
    ...
    glm::vec4 pixel = glm::affineInverse(this->transform) * glm::vec4(worldX,worldY,-1.f,1.f);
    glm::vec4 origin = glm::affineInverse(this->transform) * glm::vec4(0.f,0.f,0.f,1.f);
